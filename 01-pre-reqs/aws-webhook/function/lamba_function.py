# ---
# This is the lambda function that will be used to handle the HCP Packer webhook events
# ---
import boto3, botocore.exceptions # Import the boto3 and botocore.exceptions libraries which are used to interact with AWS services
import hashlib # Import the hashlib library which is used to create a hash of the event body
import hmac # Import the hmac library which is used to create a hash of the event body
import json # Import the json library which is used to parse the event body
import os # Import the os library which is used to get the environment variables
from datetime import datetime, timezone, timedelta # Import the datetime, timezone, and timedelta libraries which are used to handle dates and times

# --- This is the main lambda function that will be used to handle the HCP Packer webhook events
# --- This function will be called when the HCP Packer webhook event is received and will do the following:
# --- 1. Verify the HMAC signature
# --- 2. Check the event_action value to determine what to do
# --- 3. Call the appropriate function to handle the event
def lambda_handler(event, context):
    ...
    # --- If the HMAC signature is valid, then we will parse the event body and call the appropriate function to handle the event
...
# The HCP Packer webhook verification event is used to verify the HCP Packer webhook handler is working correctly.
def verify():
    """Handle the HCP Packer webhook verification event."""
    response = {"statusCode": "200", "body": "Verification successful!"}
    return response

# The HCP Packer webhook complete event is used to add metadata tags to the AMI(s).
def complete(body):
...
# The HCP Packer webhook revoke event is used to set the AMI deprecation time and add metadata tags.
def revoke(body):
...
# The HCP Packer webhook delete event is used to deregister the AMI(s) and delete their associated snapshots.
def delete(body):
...
# The HCP Packer webhook restore event is used to disable the AMI deprecation and remove the metadata tags added by the 'revoke' handler.
def restore(body):
